...
public class SpearGoblinThrowState : MonoBehaviour, ICreatureState
{
    ...
    public void ExecuteState(StateContext context, List<StateMessages> messages)
    {
        if (throwTimer <= 0)
        {
            ...
            spear.GetComponent<Rigidbody2D>().velocity = (context.playerReference.transform.position - transform.position).normalized * throwStrength + new Vector3(0, 5, 0);
